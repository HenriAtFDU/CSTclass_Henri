...
def gridSample(D,bounds,n):
    ...
    target = []
    for i in range(n):
        term = (float(i) + 0.5) * float(d)
        target.append(term)

    result = np.array(list(product(target, repeat=D)))
...
def main():
    ...
    D = 4
    n = 4
    bounds = [[0.6,1.4],[0.6,1.4],[0.6,1.4],[0.6,1.4]]
    samples = gridSample(D,bounds,n)    
    ...
    badcur = 0
    badmon = 0
    for k in range(pow(n,D)):
        para_up = para_up_init.copy()
        print('para_up_init:', para_up)
        para_up[0] = para_up[0] * W[k]
        para_up[1] = para_up[1] * X[k]
        para_up[2] = para_up[2] * Y[k]
        para_up[3] = para_up[3] * Z[k]
        print('para_up:', para_up)
        para_lo = para_lo_init
        ...
        elif (area_sum > 1.03 * area_sum_init):
            big += 1
